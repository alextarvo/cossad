...
class FurthestSampling(Function):
    ...
    def forward(ctx, xyz, m):
        ...
        # Ensure current CUDA device matches xyz.device
        with torch.cuda.device(device):
            # allocate on the same device
            idx = torch.empty((b, m), dtype=torch.int32, device=device)
            temp = torch.full((b, n), 1e10, dtype=torch.float32, device=device)
            # call the CUDA kernel on the *same* device
            pointops_cuda.furthestsampling_cuda(b, n, m, xyz, temp, idx)
        return idx
...
class Gathering(Function):
    ...
    def forward(ctx, features, idx):
        ...
        b, c, n = features.size()
        m = idx.size(1)
        with torch.cuda.device(idx.device):
...
class NearestNeighbor(Function):
    ...
    def forward(ctx, unknown: torch.Tensor, known: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        b, n, _ = unknown.size()
        m = known.size(1)
        with torch.cuda.device(unknown.device):
...
class FeatureGather(Function):
    ...
    def forward(ctx, max_feature: torch.Tensor, distribute_idx: torch.Tensor) -> torch.Tensor:
        ...
        with torch.cuda.device(max_feature.device):
